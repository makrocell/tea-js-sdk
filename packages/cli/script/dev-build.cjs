...
const {
  execSync, execScript, run, copySync,
  rootDirPath, cliDirPath,
} = require('../helper/index.cjs');
...
async function buildJs (dir) {
  ...
  if (!fs.existsSync(path.join(process.cwd(), '.skip-build'))) {
    ...
    if (!name.startsWith('tearust_')) {
      return;
    }

    console.log(chalk.yellow(`***** Building ${name} ${version} *****`));

    mkdirp.sync('build');

    fs.writeFileSync(path.join(process.cwd(), 'src/packageInfo.js'), `
...
module.exports = { name: '${name}', version: '${version}' };
    ...
    if (fs.existsSync(path.join(process.cwd(), 'public'))) {
      // TODO support
      throw 'This is a front-end package, not support.';
      // buildWebpack(dir);
    } else {
...
run(async () => {
  ...
  process.chdir(rootDirPath('packages'));
